...
create index if not exists standings_season_rank_idx on standings (season, rank);
...
create table if not exists fixtures (
  ...
  status text not null default 'NS',      -- API-Football short status (NS, 1H, HT, 2H, FT, PST, CANC...)
...
-- ---------------------------------------------------------------------------
-- sync_logs — tracks API-Football usage to stay under the 100 req/day quota
-- ---------------------------------------------------------------------------
create table if not exists sync_logs (
  id bigint generated always as identity primary key,
  resource text not null,                 -- 'standings' | 'fixtures' | 'all'
