...
async function syncEuropeanFixtures(
  ...
  for (const link of phaseLinks) {
    ...
    for (const match of parseMatchesFromHtml(html, link.url)) {
      ...
      // A two-legged tie level on aggregate (or a single-match knockout draw)
      // may have been settled on penalties — the calendar listing never
      // shows it, only the detail page does, so a tied result is worth the
      // one extra request to check.
